...
const TextBlock = ({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-lg max-w-none focus:outline-none min-h-[100px] p-4 text-lg [&_h1]:text-4xl [&_h1]:font-bold [&_h1]:my-4 [&_h2]:text-3xl [&_h2]:font-bold [&_h2]:my-3 [&_h3]:text-2xl [&_h3]:font-semibold [&_h3]:my-2 [&_ul]:list-disc [&_ul]:ml-6 [&_ol]:list-decimal [&_ol]:ml-6 [&_li]:my-1",
        spellcheck: "false",
      },
    },
  });
  ...
  // Handle double-click to show popover
  const handleDoubleClick = (event: React.MouseEvent) => {
    ...
    setPopoverPosition({
      top: rect.top - 60,
      left: Math.max(10, Math.min(rect.left, window.innerWidth - 650)),
    });
    ...
    >
      ...
      {showPopover &&
        ...
        createPortal(
          <div
            ref={popoverRef}
            className="fixed z-[9999] bg-white border border-gray-400 rounded-lg shadow-sm px-2 py-1.5 flex items-center gap-2"
            style={{
      ...
      >
        ...
        {isEditMode && (
          <div className="absolute bottom-2 right-2 text-xs text-gray-400 italic pointer-events-none">
            Double-click to format • Hold Ctrl to drag
          </div>
        )}
      </div>
    </div>
