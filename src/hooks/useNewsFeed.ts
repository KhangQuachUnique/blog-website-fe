...
const getOrCreateSessionSeed = (): number => {
  ...
  if (!__newsfeed_seed_reload_handled) {
    try {
      const navEntries = (performance && (performance.getEntriesByType?.("navigation") || [])) as PerformanceEntryList;
      const perfNav = (performance as unknown as { navigation?: { type: number } }).navigation;
      const navType = (navEntries[0] as PerformanceNavigationTiming | undefined)?.type ?? perfNav?.type;
...
export const useGetNewsFeed = () => {
  ...
  return useInfiniteQuery<IGetNewsfeedResponseDto>({
    ...
    queryFn: ({ pageParam }) => getNewsfeed((pageParam as string | undefined) ?? undefined, sessionSeed),
