      ...
      return (
        <div className="flex flex-col items-center justify-center min-h-[300px] text-[#F295B6]">
          <svg className="animate-spin h-8 w-8 mb-2" viewBox="0 0 24 24" fill="none">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="#F295B6" strokeWidth="4" />
            <path className="opacity-75" fill="#F295B6" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z" />
          </svg>
    ...
    if (error || !post) {
      return (
        <div className="flex flex-col items-center justify-center min-h-[300px] text-red-500">
          ...
          <span className="text-sm mt-1">Vui lòng thử lại hoặc kiểm tra kết nối.</span>
        </div>
      );
    }
    ...
    if (post.author.id !== user?.id) {
      return (
        <div className="flex flex-col items-center justify-center min-h-[300px] text-yellow-500">
          ...
          <span className="text-lg font-semibold">Bạn không có quyền chỉnh sửa bài viết này</span>
          <span className="text-sm mt-1">Chỉ tác giả mới được chỉnh sửa nội dung.</span>
        </div>
      );
    }
