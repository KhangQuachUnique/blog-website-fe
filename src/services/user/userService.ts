...
import type { UserProfile, UpdateProfileData, ChangePasswordData, BlockedUser } from '../../types/user.types';
 ...
 */
export const getUserProfile = async (userId: number, viewerId?: number): Promise<UserProfile> => {
  const params = viewerId ? { viewerId } : {};
  const response = await axiosCustomize.get(`/users/${userId}/profile`, { params });
  return unwrap(response);
};
...
export const requestPasswordReset = async (email: string): Promise<{ message: string }> => {
  const response = await axiosCustomize.post('/users/forgot-password', { email });
  return unwrap(response);
};
 ...
 * Chặn người dùng
 */
export const blockUser = async (userId: number): Promise<{ message: string }> => {
